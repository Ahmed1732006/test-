...
(() => {
  ...
  async function downloadStorageBlob(bucket, path, name) {
    await requireSignedInForStorage('تحميل الملف');
  ...
  downloadMaterial = async function(item) {
    try {
      ...
      await downloadStorageBlob('materials', rawPath, item?.fileName || item?.name || deriveOriginalFileName(rawPath) || 'محاضرة');
    } catch (e) {
  ...
  async function downloadMemberBatch(batchId) {
    ...
    try {
      for (const f of files) {
        ...
        if (r.error) throw r.error;
        zip.file(f.original_name, r.data);
      }
